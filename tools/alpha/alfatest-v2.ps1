...
function Install-Vdd {
    ...
    $driverZip = Join-Path $WorkDir 'vdd.zip'
    $nefconZip = Join-Path $WorkDir 'nefcon.zip'
    Write-Log "Runtime root: $RuntimeRoot"
    Write-Log "Downloading VDD $DriverVersion into repository runtime..." Cyan
    Invoke-WebRequest -Uri $DriverUrl -OutFile $driverZip -UseBasicParsing
    Assert-Hash -Path $driverZip -Expected $DriverSha256
    Write-Log "Downloading NefCon $NefConVersion into repository runtime..." Cyan
    ...
    foreach ($required in @($infPath,$catPath,$nefconExe)) {
        if (-not (Test-Path -LiteralPath $required)) { throw "Required installation file not found: $required" }
    }
    ...
    $devices = @(Get-VddDevices)
    if ($devices.Count -ne 1) { throw "Expected exactly one VDD device after installation, found $($devices.Count)." }
...
function Get-Displays {
    ...
    $items = @(); [uint32]$i=0
    while ($true) {
        $d = New-Object Vmu.AlphaDisplayApi2+DISPLAY_DEVICE; $d.cb=[Runtime.InteropServices.Marshal]::SizeOf($d)
        if (-not [Vmu.AlphaDisplayApi2]::EnumDisplayDevices($null,$i,[ref]$d,0)) { break }
        $items += [pscustomobject]@{ DeviceName=$d.DeviceName; DeviceString=$d.DeviceString; Attached=(($d.StateFlags -band 1)-ne 0) }
        $i++
    }
    return $items
}
...
function Get-Mode {
    ...
    Ensure-DisplayApi
    $m=New-Object Vmu.AlphaDisplayApi2+DEVMODE; $m.dmSize=[Runtime.InteropServices.Marshal]::SizeOf($m)
    $index=if($Registry){-2}else{-1}
    if(-not [Vmu.AlphaDisplayApi2]::EnumDisplaySettings($DeviceName,$index,[ref]$m)){throw "Cannot read mode for $DeviceName"}
    return $m
}

function Set-Mode {
    param([string]$DeviceName,[uint32]$Width,[uint32]$Height,[uint32]$RefreshRate)
    $m=Get-Mode $DeviceName; $m.dmPelsWidth=$Width; $m.dmPelsHeight=$Height; $m.dmDisplayFrequency=$RefreshRate; $m.dmFields=0x80000 -bor 0x100000 -bor 0x400000
...
function Disconnect-Display {
    ...
    $m=Get-Mode $DeviceName; $m.dmPelsWidth=0; $m.dmPelsHeight=0; $m.dmFields=0x20 -bor 0x80000 -bor 0x100000
    $r=[Vmu.AlphaDisplayApi2]::ChangeDisplaySettingsEx($DeviceName,[ref]$m,[IntPtr]::Zero,1,[IntPtr]::Zero)
...
function Reconnect-Display {
    ...
    if($m.dmPelsWidth-eq 0 -or $m.dmPelsHeight-eq 0){$m.dmPelsWidth=1920;$m.dmPelsHeight=1080;$m.dmDisplayFrequency=60}
...
function Open-DisplaySettings { Start-Process 'ms-settings:display' | Out-Null; Start-Sleep -Seconds 2 }
...
try {
    ...
    if($existing.Count -gt 0){ if(-not (Remove-VddInstallation)){throw 'Could not establish clean baseline.'} }
    Install-Vdd
    ...
    Set-Mode $name 1920 1080 60; Write-Log 'Set 1920x1080 @ 60 Hz.' Green
    Set-Mode $name 3840 2160 60; Write-Log 'Set 3840x2160 @ 60 Hz.' Green
    Open-DisplaySettings
    if(Ask-User 'Does the virtual monitor now show 3840x2160 and remain usable?'){ $Results.DynamicResolution='PASS' } else { $Results.DynamicResolution='FAIL' }
    Set-Mode $name 1920 1080 60; Write-Log 'Returned to 1920x1080 @ 60 Hz.' Green

    Write-Section 'TEST 2: DISCONNECT / RECONNECT WITHOUT UNINSTALL'
    Disconnect-Display $name; Open-DisplaySettings
    if(Ask-User 'Is the virtual monitor still known by Windows but disconnected from the desktop (not Extend/Clone)?'){ $disconnectOk=$true } else { $disconnectOk=$false }
    Reconnect-Display $name; Open-DisplaySettings
    if(Ask-User 'Was the same virtual monitor reconnected without reinstalling the driver?'){ $reconnectOk=$true } else { $reconnectOk=$false }
    ...
    if($uninstallOk -and $userOk){$Results.UninstallFirstAttempt='PASS'}else{$Results.UninstallFirstAttempt='FAIL'}
}
catch {
    ...
    foreach($entry in $Results.GetEnumerator()){Write-Log ("{0}: {1}" -f $entry.Key,$entry.Value) $(if($entry.Value -eq 'PASS'){'Green'}elseif($entry.Value -eq 'FAIL'){'Red'}else{'Yellow'})}
