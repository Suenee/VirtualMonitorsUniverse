...
internal sealed class SettingsForm : Form
{
    ...
    public SettingsForm(string settingsPath, Action<ServerSettings, ServerSettings> saved, Func<int, bool> isOwnedListener, Icon icon)
    {
        ...
        var general = new GroupBox { Text = "Logging", AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, Dock = DockStyle.Fill, Padding = new Padding(10, 8, 10, 10), Margin = new Padding(0, 10, 0, 0) };
        ...
        buttons.Controls.Add(save); buttons.Controls.Add(cancel); root.Controls.Add(buttons);
    ...
    private static TableLayoutPanel CreateTwoColumnLayout()
    {
        ...
        layout.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 140)); layout.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 220)); return layout;
    }
    private static void AddTwoColumnRow(TableLayoutPanel layout, string text, Control control)
    {
        ...
        control.Margin = new Padding(0, 2, 0, 4); control.Anchor = AnchorStyles.Left; layout.Controls.Add(control, 1, row);
    }
    private static Control WrapWithSuffix(Control control, string suffix)
    {
        ...
        control.Margin = Padding.Empty; panel.Controls.Add(control); panel.Controls.Add(new Label { Text = suffix, AutoSize = true, Margin = new Padding(8, 5, 0, 0) }); return panel;
    }
    ...
    {
        var row = layout.RowCount++; layout.Controls.Add(new Label { Text = name, AutoSize = false, Width = 112, Height = 27, TextAlign = ContentAlignment.MiddleLeft, Anchor = AnchorStyles.Left, Margin = Padding.Empty }, 0, row);
        var border = new Panel { AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, Padding = new Padding(1), Margin = Padding.Empty, BackColor = SystemColors.Control }; border.Controls.Add(portBox); _portBorders[portBox] = border;
    ...
    private void ApplyInterfaceDependency() { var vmuAny = GetInterface(_vmuInterface).Equals("any", StringComparison.OrdinalIgnoreCase); if (vmuAny) _webInterface.SelectedItem = "any"; _tips.SetToolTip(_webInterface, vmuAny ? "Web Server must listen on any interface while VMU Server listens on any interface." : string.Empty); }

    private void SaveSettings()
    {
        ApplyInterfaceDependency(); if (!ValidatePorts(true)) return;
        ...
        settings.Exit.RestoreServices = _restoreServices.Checked; settings.Save(_settingsPath); _saved(_originalSettings, settings); Close();
    }

    private bool ValidatePorts(bool showDialog)
    {
        ClearAllPortErrors(); var endpoints = new[] { new EndpointInput("VMU Server", _vmuPort), new EndpointInput("Web Server", _webPort), new EndpointInput("Web Socket", _socketPort) }; var errors = new List<string>();
        foreach (var group in endpoints.GroupBy(x => x.Port).Where(x => x.Count() > 1)) { var names = string.Join(" and ", group.Select(x => x.Name)); foreach (var endpoint in group) SetPortError(endpoint.PortBox, $"Port {endpoint.Port} is configured for both {names}."); errors.Add($"Port {group.Key} is configured for more than one VMU service."); }
        var activePorts = IPGlobalProperties.GetIPGlobalProperties().GetActiveTcpListeners().Select(x => x.Port).ToHashSet();
        foreach (var endpoint in endpoints) { if (!activePorts.Contains(endpoint.Port) || _isOwnedListener(endpoint.Port)) continue; var message = $"{endpoint.Name} port {endpoint.Port} is already used by another TCP listener on this computer."; SetPortError(endpoint.PortBox, message); errors.Add(message); }
        if (GetInterface(_vmuInterface).Equals("any", StringComparison.OrdinalIgnoreCase) && !GetInterface(_webInterface).Equals("any", StringComparison.OrdinalIgnoreCase)) errors.Add("Web Server interface must be 'any' while VMU Server interface is 'any'.");
        if (showDialog && errors.Count > 0) MessageBox.Show(string.Join(Environment.NewLine, errors.Distinct()), "VMU Settings", MessageBoxButtons.OK, MessageBoxIcon.Warning); return errors.Count == 0;
    }
    private void SetPortError(NumericUpDown port, string message) { if (_portBorders.TryGetValue(port, out var border)) border.BackColor = Color.Firebrick; port.ForeColor = Color.Firebrick; _tips.SetToolTip(port, message); }
    private void ClearAllPortErrors() { foreach (var port in _portBorders.Keys) { if (_portBorders.TryGetValue(port, out var border)) border.BackColor = SystemColors.Control; port.ForeColor = SystemColors.WindowText; _tips.SetToolTip(port, string.Empty); } }
