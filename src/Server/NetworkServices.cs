...
internal sealed record MonitorCreateRequest(string FriendlyName, int Width, int Height, int RefreshRate, bool Portrait, bool Connect);
...
internal sealed class WebServerService : NetworkService
{
    ...
    private readonly MonitorThumbnailService _thumbnailService = new();
    ...
    protected override void ConfigureApplication(WebApplication app)
    {
        ...
        app.MapGet("/api/monitors/{id}/thumbnail", GetMonitorThumbnailAsync);
...
<form id="form" class="settingsform"><table class="settings"><thead><tr><th>Service</th><th>Interface</th><th>Port</th></tr></thead><tbody>
...
<div class="settingrow"><label for="retention">Log Retention</label><input id="retention" type="number" min="1" max="3650"><span>days</span></div>
...
function thumbnail(m){if(!m.connected)return '<div class="screen disconnected"></div>';const id=encodeURIComponent(m.configuration.vmuId);return `<div class="screen"><img class="monitorpreview" data-thumbnail="/api/monitors/${id}/thumbnail" src="/api/monitors/${id}/thumbnail?t=${Date.now()}" alt="${esc(m.configuration.friendlyName)} preview" onerror="this.classList.add('failed')"></div>`;}
function refreshThumbnails(){if(document.hidden)return;document.querySelectorAll('.monitorpreview').forEach(img=>{img.classList.remove('failed');img.src=img.dataset.thumbnail+'?t='+Date.now();});}
async function loadMonitors(){const list=await fetch('/api/monitors',{cache:'no-store'}).then(r=>r.json());monitorGrid.innerHTML=list.map(m=>`<a class="monitorcard" href="/monitors/${encodeURIComponent(m.configuration.vmuId)}"><div class="monitorpic">${thumbnail(m)}<div class="stand"></div></div><h3>${esc(m.configuration.friendlyName)}</h3><div>${m.width} × ${m.height}</div><div><span class="dot ${m.connected?'on':'off'}"></span> ${m.connected?'On':'Off'}</div></a>`).join('')+`<a class="monitorcard addmonitor" href="/monitors/new"><div class="plus">+</div><h3>Add Monitor</h3></a>`;}loadMonitors();setInterval(refreshThumbnails,6000);
</script>
""";
    ...
    {
        var body = """
<div class="page"><h1>Add Monitor</h1><div id="error" class="error"></div>
<form id="newMonitor" class="properties newmonitor">
<label>Name <input id="friendlyName" value="New Monitor"></label>
...
<label>Refresh Rate <input id="refreshRate" type="number" min="1" max="1000" value="60"></label>
<label>Orientation <select id="portrait"><option value="false">Landscape</option><option value="true">Portrait</option></select></label>
<div class="actions"><button type="submit" data-connect="false">Install</button><button type="submit" data-connect="true">Install and Connect</button><a class="buttonlink" href="/monitors">Cancel</a></div>
</form>
<div id="operation" class="operation hidden"><h2 id="operationTitle">Installing Monitor...</h2><div class="progress"><div id="operationBar"></div></div><p id="operationText">Windows may ask for administrator confirmation.</p></div></div>
<script>
let connectAfterInstall=false;document.querySelectorAll('#newMonitor button[type=submit]').forEach(b=>b.onclick=()=>connectAfterInstall=b.dataset.connect==='true');
newMonitor.onsubmit=async e=>{e.preventDefault();error.textContent='';const [width,height]=resolution.value.split('x').map(Number);newMonitor.classList.add('hidden');operation.classList.remove('hidden');const started=Date.now();const estimate=20000;const timer=setInterval(()=>operationBar.style.width=Math.min(92,Math.round((Date.now()-started)/estimate*92))+'%',120);try{const r=await fetch('/api/monitors',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({friendlyName:friendlyName.value,width,height,refreshRate:+refreshRate.value,portrait:portrait.value==='true',connect:connectAfterInstall})});const x=await r.json();if(!r.ok)throw new Error(x.error||'Monitor could not be installed.');clearInterval(timer);operationBar.style.width='100%';operationTitle.textContent='Monitor Installed';operationText.textContent=connectAfterInstall?'Monitor is installed and connected.':'Monitor is installed and disconnected.';setTimeout(()=>location.href='/monitors/'+encodeURIComponent(x.configuration.vmuId),350);}catch(ex){clearInterval(timer);operation.classList.add('hidden');newMonitor.classList.remove('hidden');error.textContent=ex.message;}};
</script>
""";
...
async function action(name,question){if(question&&!confirm(question))return;const r=await fetch('/api/monitors/'+encodeURIComponent(id)+'/'+name,{method:'POST'});if(!r.ok){const x=await r.json();alert(x.error||'Monitor operation failed.');return false;}return true;}
connect.onclick=async()=>{if(await action('connect'))await load();};disconnect.onclick=async()=>{if(await action('disconnect',`Are you sure you want to disconnect monitor '${current.configuration.friendlyName}'?`))await load();};uninstall.onclick=async()=>{if(await action('uninstall',`Are you sure you want to uninstall monitor '${current.configuration.friendlyName}'?\n\nThe virtual monitor will be removed from Windows.`))location.href='/monitors';};load().catch(e=>error.textContent=e.message);
</script>
""";
        return HtmlShell($"Monitor {safeId}", body);
    }

    private async Task<IResult> GetMonitorThumbnailAsync(string id, CancellationToken cancellationToken)
    {
        var monitor = _monitorService.Get(id);
        if (monitor is null) return Results.NotFound();
        if (!monitor.Connected || string.IsNullOrWhiteSpace(monitor.DeviceName))
        {
            _thumbnailService.Invalidate(id);
            return Results.Conflict(new { error = "Monitor is not connected." });
        }

        try
        {
            var bytes = await _thumbnailService.GetThumbnailAsync(id, monitor.DeviceName, cancellationToken);
        ...
        {
            return Results.Problem(title: "Monitor thumbnail capture failed", detail: ex.Message, statusCode: StatusCodes.Status503ServiceUnavailable);
        }
    }
    ...
    private async Task<IResult> CreateMonitorAsync(HttpRequest request)
    {
        try
        {
            ...
            var monitor = _monitorService.Create(input.FriendlyName, input.Width, input.Height, input.RefreshRate, input.Portrait, input.Connect);
            return Results.Created($"/api/monitors/{monitor.Configuration.VmuId}", monitor);
        }
    ...
    {
        try { _monitorService.Uninstall(id); _thumbnailService.Invalidate(id); return Results.NoContent(); }
        catch (Exception ex) { return Results.BadRequest(new { error = ex.Message }); }
    }
