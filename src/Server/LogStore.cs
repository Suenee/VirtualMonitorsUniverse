...
internal sealed class LogStore
{
    ...
    public LogCount Count(string? search = null, IReadOnlyCollection<string>? services = null)
    {
        ...
        using var total = connection.CreateCommand(); total.CommandText = "SELECT COUNT(*) FROM log_entries"; var totalCount = Convert.ToInt64(total.ExecuteScalar());
        using var filtered = connection.CreateCommand(); var where = BuildWhere(filtered, search, services, 0); filtered.CommandText = $"SELECT COUNT(*) FROM log_entries WHERE {where}"; var filteredCount = Convert.ToInt64(filtered.ExecuteScalar());
        return new LogCount(totalCount, filteredCount);
    }
    ...
    private static string BuildWhere(SqliteCommand command, string? search, IReadOnlyCollection<string>? services, long afterId)
    {
        ...
        if (services is not null)
        {
            ...
            if (names.Length == 0) where.Add("1=0");
            else { var parameters = new List<string>(names.Length); for (var i=0;i<names.Length;i++) { var p=$"$service{i}"; parameters.Add(p); command.Parameters.AddWithValue(p,names[i]); } where.Add($"service IN ({string.Join(',',parameters)})"); }
        }
    ...
    {
        var result=new List<LogEntry>(); using var reader=command.ExecuteReader(); while(reader.Read()) { var utc=DateTime.Parse(reader.GetString(1),null,System.Globalization.DateTimeStyles.RoundtripKind); result.Add(new LogEntry(reader.GetInt64(0),utc.ToLocalTime(),reader.GetString(2),reader.GetString(3),reader.IsDBNull(4)?null:reader.GetString(4),reader.GetString(5),reader.GetString(6),reader.IsDBNull(7)?null:reader.GetString(7))); } return result;
    }
