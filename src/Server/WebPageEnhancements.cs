...
(() => {
  ...
  live.addEventListener('click',async e=>{if(mouseHandoff||reconnecting||live.classList.contains('hidden'))return;if(mouseAllowed!==true){toast(mouseAllowed===false?'Mouse is disabled in Monitor Properties.':'Mouse control is not ready yet.',true);return;}const point=renderedPoint(e);if(!point)return;mouseHandoff=true;try{const ox=e.screenX-e.clientX,oy=e.screenY-e.clientY;const left=Math.round(ox+point.rect.left),top=Math.round(oy+point.rect.top),right=Math.round(ox+point.rect.right),bottom=Math.round(oy+point.rect.bottom);await postMouse({type:'portalbounds',x:null,y:null,button:left,delta:top});await postMouse({type:'portal',x:point.x,y:point.y,button:right,delta:bottom});}catch(error){toast(error?.message||'Terminal mouse input failed.',true);}finally{mouseHandoff=false;}});
