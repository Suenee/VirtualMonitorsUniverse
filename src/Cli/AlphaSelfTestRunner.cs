...
internal static class AlphaSelfTestRunner
{
    public static int Run()
    {
        ...
        try
        {
            ...
            createdAny = true;
            if (!WaitUntil(() => ActiveVirtual(displays).Length == 1, TimeSpan.FromSeconds(20)))
                throw new TimeoutException("VDD A did not appear.");
            ...
            AlphaSelfTestVdd.InstallOne(payload);
            if (!WaitUntil(() => ActiveVirtual(displays).Length == 2, TimeSpan.FromSeconds(20)))
                throw new TimeoutException("VDD B did not appear.");
            ...
            if (reconnected is null || reconnected.Width != original.Width || reconnected.Height != original.Height)
                throw new InvalidOperationException("Reconnect did not preserve the VDD-A mode.");
            report.Status("RECONNECT ISOLATION .... ", "PASS");

            report.Status("UNINSTALL VDD A ........ ", "RUN");
            AlphaSelfTestVdd.RemoveOne(a.DeviceID);
            if (!WaitUntil(() => !DeviceIsAttached(displays, a.DeviceID), TimeSpan.FromSeconds(5)))
                throw new InvalidOperationException("Could not uninstall VDD-A on the first attempt.");
            if (!FindByDeviceId(displays, b.DeviceID).IsAttached)
            ...
            if (!WaitUntil(() => !DeviceIsAttached(displays, b.DeviceID), TimeSpan.FromSeconds(5)))
    ...
    private static WindowsDisplayInfo FindByDeviceId(WindowsDisplayModeService service, string deviceId) =>
        service.GetDisplays().FirstOrDefault(item => string.Equals(item.DeviceID, deviceId, StringComparison.OrdinalIgnoreCase))
        ?? throw new InvalidOperationException($"Display device {deviceId} is missing.");

    private static bool DeviceIsAttached(WindowsDisplayModeService service, string deviceId) =>
        service.GetDisplays().Any(item => string.Equals(item.DeviceID, deviceId, StringComparison.OrdinalIgnoreCase) && item.IsAttached);
    ...
    private sealed class Reporter : IDisposable
    {
        ...
        {
            if (color.HasValue) CliConsole.WriteLine(text, color.Value); else Console.WriteLine(text);
